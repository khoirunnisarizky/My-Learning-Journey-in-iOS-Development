...
class KanbanColumnCell: UICollectionViewCell{
    ...
    var columnInteraction: UIDragInteraction! //bikin instance dg nama column Interaction belum diinisialisasi
...
extension KanbanColumnCell: UIDragInteractionDelegate{
    ...
    private func configureInteraction(){
        ...
        columnInteraction = UIDragInteraction(delegate: self)
        addInteraction(columnInteraction) //method dr kelas UIView
    }
    
    //TODO: 3. Implement How the drag functionality behave, ini method dr UIDragInteractionDelegate
    func dragInteraction(_ interaction: UIDragInteraction, itemsForBeginning session: UIDragSession) -> [UIDragItem] {
        // Write Some Code Here -- UIDragItemnya itu apa? mau string kah? URL kah? image kah?
        var stringResult = "Kanban Log"
        for row in rows {
            stringResult += "\n \(row.title)"
        }
        
        let itemProvider = NSItemProvider(object: stringResult as NSString)
